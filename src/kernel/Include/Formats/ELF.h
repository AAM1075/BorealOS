...
// Yes, we use defines here instead of constexpr/enums/etc because I am lazy, and the linux elf.h is in C, not C++, so it uses defines.
// This also makes it easier to write since it matches a very well-known specification.
...
namespace Formats {
    // This is mostly stolen from linux/include/uapi/linux/elf.h, but simplified a bit.
...
#define EI_NIDENT	16
    ...
    public:
        ...
        explicit ELF(const uint8_t *data, size_t size, Endianness systemEndianess = Endianness::LittleEndian);
